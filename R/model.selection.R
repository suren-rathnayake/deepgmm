...
model.selection=function(y,k,layers,seeds=3,it=50,eps=0.001,criterion="BIC")
{
  bic.best=Inf
  aic.best=Inf
  p=dim(y)[2]
  ...
  if (layers==1) {
    ...
    for (i in 1:seeds) for (rr in 1:pp) {
      ...
      bic.best=out$bic
        ...
        if (criterion=="AIC") if (out$aic<aic.best) {out.best=out
        aic.best=out$aic
        }
    ...
    if (criterion=="BIC") index=which(bic==min(bic),arr.ind=TRUE)[1,]
    if (criterion=="AIC") index=which(aic==min(aic),arr.ind=TRUE)[1,]
    message("Best Fit: \n")
...
for (i in 1:seeds) for (kk in 1:nrow(k))  for (rr in 1:nrow(r)) {
  ...
  if (criterion=="AIC") index=which(aic==min(aic),arr.ind=TRUE)[1,]
  message("Best Fit: \n")
  cat(paste("Seed=",index[1]," k=",paste(k[index[2],],collapse=" ")," r=",paste(r[index[3],],collapse=" "),
            " BIC:",round(out.best$bic,2)," AIC:",round(out.best$aic,2)))                  
  }## end layers=2
   ...
   k=rbind(c(k,1,1),c(k,2,1),c(k,3,1),c(k,1,2),c(k,2,2),c(k,3,2),c(k,1,3),c(k,2,3),c(k,3,3))
